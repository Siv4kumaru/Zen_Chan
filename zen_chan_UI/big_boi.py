...
def copy_tat_data_boi(profile):
    try:
        ...
        history_db_path = os.path.join(user_profile, "Appdata","Local","Google","Chrome","User Data", profile,"History")
...
def arrange_tat_data_boi(time_filter="today"):
    ...
    if time_filter == "today":
        ...
        start_ts = datetime_to_webkit(start)
        where_clause = f"WHERE visits.visit_time >= {start_ts}"
        ...
        start_ts = datetime_to_webkit(start)
        end_ts = datetime_to_webkit(end)
        where_clause = f"WHERE visits.visit_time BETWEEN {start_ts} AND {end_ts}"
...
def load_profile_data(profile,time="today"):
    ...
    yield "data: > Mood mapping in progress .....\n\n"
    # Apply it once
...
# for msg in load_profile_data("Profile 11"):
#     print(msg)
